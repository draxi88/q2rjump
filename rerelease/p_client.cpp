...
void SP_info_player_deathmatch(edict_t *self)
{
	...
	if (level.jump_start_spawned) {
		G_FreeEdict(self);
		return;
	}
	level.jump_start_spawned = true;
...
void ClientThink(edict_t *ent, usercmd_t *ucmd)
{
	...
	else
	{
		...
		if (client->latched_buttons & BUTTON_ANY) {
			if (!client->jump_started) {
				client->jump_started = true;
				client->jump_timer_start = level.time.seconds();

			}
		}
		if (!client->jump_started) {
			//gi.LocBroadcast_Print(PRINT_HIGH, "jump_timer: {}\n", client->jump_timer);
			G_FmtTo(jump_timer, "0.00");
		}
		else if (client->jump_finished) {
			G_FmtTo(jump_timer, "{:.2f}", client->jump_timer);
		}
		else {
			//gi.LocBroadcast_Print(PRINT_HIGH, "jump_timer: {}\n", level.time.seconds() - client->jump_timer_start);
			G_FmtTo(jump_timer, "{:.2f}", level.time.seconds() - client->jump_timer_start);
		}
		gi.configstring(CONFIG_JUMP_TIMER, jump_timer);
